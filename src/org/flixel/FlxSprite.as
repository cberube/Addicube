	...
	*/
	public class FlxSprite extends FlxObject
	{
		...
		*/
		...
		{
			//	This was originally X -= ... and Y -= ... -- seems to be incorrect
		 ...
		 * @return	Whether or not the point overlaps this object.
		 */
		public function overlapsPointScreenSpace(X:Number,Y:Number,PerPixel:Boolean = false):Boolean
		{
			getScreenXY(_point);
			
			if(PerPixel)
				return _framePixels.hitTest(new Point(0,0),0xFF,new Point(X,Y));
			else if((X <= _point.x) || (X >= _point.x+frameWidth) || (Y <= _point.y) || (Y >= _point.y+frameHeight))
				return false;
			return true;
		}
		...
		public function playWithoutInterruption(AnimName:String,Force:Boolean=false):void
		{
			if(!Force && (_curAnim != null) && (AnimName == _curAnim.name)) return;
			_caf = 0;
			var al:uint = _animations.length;
			for(var i:uint = 0; i < al; i++)
			{
				if(_animations[i].name == AnimName)
				{
					...
					_curFrame = (_curFrame % _curAnim.frames.length);
